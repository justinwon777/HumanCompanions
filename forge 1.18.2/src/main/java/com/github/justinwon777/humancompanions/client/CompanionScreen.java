...
public class CompanionScreen extends AbstractContainerScreen<CompanionContainer> implements MenuAccess<CompanionContainer> {
    ...
    protected void init() {
        ...
        this.huntingButton = addRenderableWidget(new CompanionButton("hunting", col2, row1, 16,
                12, 0, 0,13,
                HUNTING_BUTTON,
                btn -> {
                    PacketHandler.INSTANCE.sendToServer(new SetHuntingPacket(companion.getId()));
                }));
        this.patrolButton = addRenderableWidget(new CompanionButton("patrolling", col1, row2, 16,
                12,
                0, 0
                ,13,
                PATROL_BUTTON,
        ...
        if (companion instanceof Archer || companion instanceof Arbalist) {
            this.stationeryButton = addRenderableWidget(new CompanionButton("stationery", col2, row2, 16,
                    12,
                    0, 0
                    , 13,
                    STATIONERY_BUTTON,
    ...
    protected void renderTooltip(PoseStack stack, int x, int y) {
        ...
        if (this.releaseButton.isHoveredOrFocused()) {
            ...
            tooltips.add(new TextComponent("Releases companion from your command. It can be tamed again.").withStyle(ChatFormatting.GRAY).withStyle(ChatFormatting.ITALIC));
